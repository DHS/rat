...
?>
	...
	if ($this->config->items['comments']['enabled'] == TRUE) {
		echo ' &middot <a href="#" onclick="document.getElementById(\'comment_form_' . $this->item->k . '\').style.visibility = \'visible\'; document.getElementById(\'comment_form_' . $this->item->k . '\').style.height = \'auto\'; document.getElementById(\'comment_form_' . $this->item->k . '\').content.focus(); return false;">' . $this->config->items['comments']['name'].'</a>';
	}

	if ($this->config->items['likes']['enabled'] == TRUE) {
		if ($i_like == TRUE) {
			echo ' &middot <span id="like_link_' . $this->item->k . '"><a href="#" onclick="like_remove(\''.BASE_DIR.'\', ' . $this->item->k . ', \''.str_replace("'", "\'", $this->config->items['likes']['name']).'\', \''.str_replace("'", "\'", $this->config->items['likes']['opposite_name']).'\'); return false;">' . $this->config->items['likes']['opposite_name'].'</a></a></span>';
		} else {
			echo ' &middot <span id="like_link_' . $this->item->k . '"><a href="#" onclick="like_add(\''.BASE_DIR.'\', ' . $this->item->k . ', \''.str_replace("'", "\'", $this->config->items['likes']['name']).'\', \''.str_replace("'", "\'", $this->config->items['likes']['opposite_name']).'\'); return false;">' . $this->config->items['likes']['name'].'</a></a></span>';
		}
